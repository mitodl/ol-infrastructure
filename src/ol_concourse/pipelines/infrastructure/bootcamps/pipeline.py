...
pulumi_jobs = pulumi_jobs_chain(
    pulumi_code=bootcamps_pulumi_code,
    stack_names=[
        "applications.bootcamps.CI",
        "applications.bootcamps.QA",
        "applications.bootcamps.Production",
    ],
    project_name="ol-infrastructure-bootcamps-application",
    project_source_path=PULUMI_CODE_PATH.joinpath("applications/bootcamps/"),
)

mm_fragment = PipelineFragment.combine_fragments(pulumi_jobs)
bootcamps_pipeline = Pipeline(
    resources=[*mm_fragment.resources, bootcamps_pulumi_code],
...
if __name__ == "__main__":
    ...
    sys.stdout.write("\n")
    sys.stdout.write(
        "fly -t pr-inf set-pipeline -p pulumi-bootcamps -c definition.json"
    )
