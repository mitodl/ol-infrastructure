...
1. Mirror the change in `capture/<property>.capture.ts`. It follows the specs step for
   ...
   Keycloak screen.
2. Regenerate the images, one property at a time. Name the property's capture file,
   because the config takes a single `CANARY_BASE_URL`, and running every property's
   capture against one host would point each property at the wrong target:

   ```bash
   cd src/ol_concourse/pipelines/canaries
   CANARY_BASE_URL=https://rc.learn.mit.edu \
     npx playwright test -c docs/capture/playwright.capture.config.ts docs/capture/mit-learn.capture.ts
   ```
...
`capture/<property>.capture.ts`, written to `images/<property>/<journey>/`, and is
regenerated by naming that file with its own `CANARY_BASE_URL`. Nothing above changes. Frames are viewport-only JPEG, around 60KB each. Keep them that way, since
