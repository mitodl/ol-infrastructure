...
def build_meta_job(release_name):
    if release_name == "meta":
        pipeline_definition_path = (
            "src/ol_concourse/pipelines/open_edx/codejail_v3/meta.py"
        )
        pipeline_team = "main"
        pipeline_id = "self"
    else:
...
if __name__ == "__main__":
    ...
    pipeline_json = pipeline_json_with_user_data(
        ...
        user_data={
            "description": (
                "Regenerates and applies the per-release codejail pipelines "
    ...
    sys.stdout.write(
        "\nfly -t <target> set-pipeline -p dagger-pulumi-codejail-meta -c definition.json"  # noqa: E501
    )
