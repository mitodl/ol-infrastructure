...
| [`org.openedx.frontend.learning.notifications_discussions_sidebar.v1`](https://github.com/mitodl/frontend-app-learning/tree/master/src/plugin-slots/NotificationsDiscussionsSidebarSlot) | MITx Online / MIT Learn Learning MFE | Replaces the default notifications/discussions sidebar with a coordinated sidebar that manages both the existing sidebar and the AskTim Chatbot. The AskTim Chatbot opens in the sidebar slot when triggered via postMessage, and automatically coordinates with the existing sidebar to ensure only one is visible at a time. | See [`SidebarAIDrawerCoordinator.jsx`](./SidebarAIDrawerCoordinator.jsx) and [`AIDrawerManagerSidebar.jsx`](./AIDrawerManagerSidebar.jsx). The coordinator listens for `smoot-design::ai-drawer-open` and `smoot-design::tutor-drawer-open` messages to trigger the AI drawer. |
| mitxonline-styles.scss | MITx Online / MIT Learn Header | Matches the style of MITx Online / MIT Learn Header with MIT Learn Marketing Site | Header Desktop View ![](./images/header_desktop_view.png) Header Mobile View ![](./images/header_mobile_view.png) |
