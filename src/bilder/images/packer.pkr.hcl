...
build {
  ...
  provisioner "shell-local" {
    ...
    inline           = ["pyinfra --data ssh_strict_host_key_checking=off --sudo --user ${build.User} --port ${build.Port} --key /tmp/packer-session-${build.ID}.pem ${build.Host} ${path.root}/${var.app_name}/deploy.py"]
  }
