...
def create_k8s_configmaps(  # noqa: PLR0915
    ...
    if meilisearch_config.get_bool("enabled"):
        ...
        cms_interpolated_config["MEILISEARCH_COURSE_INDEXING"] = (
    ...
    # Settings module mounted over <service>/envs/mitol/production.py, which
    # DJANGO_SETTINGS_MODULE points at instead of <service>.envs.production.
    #
    # It exists solely to undo an upstream override. Both lms/envs/production.py and
    # cms/envs/production.py replace SEARCH_ENGINE with search.elastic.ElasticSearchEngine
    # whenever certain flags are set -- CMS on ENABLE_COURSEWARE_INDEX/ENABLE_LIBRARY_INDEX,
    # LMS on ENABLE_COURSEWARE_SEARCH/ENABLE_DASHBOARD_SEARCH/ENABLE_COURSE_DISCOVERY/
    # ENABLE_TEAMS -- discarding the SEARCH_ENGINE we set in the interpolated config.
    # Setting those flags under FEATURES does not avoid it: FEATURES is a
    # FeaturesProxy(globals()), so a FEATURES entry *is* the top-level module global the
    # override tests. Re-asserting after `import *` is the only hook available without
    # rebuilding the image. Reported upstream as
    # https://github.com/openedx/openedx-platform/issues/38991 -- drop this once that lands.
    #
    # The same file works for both services because the import is relative: under
    # lms/envs/mitol/ it resolves to lms.envs.production, under cms/envs/mitol/ to
    # cms.envs.production.
    settings_override_config_name = "ol-settings-override"
    settings_override_module = textwrap.dedent(
        """\
        \"\"\"Deployment settings entrypoint. Generated by Pulumi; do not edit in-place.\"\"\"
    ...
    if (
        ...
        settings_override_module += textwrap.dedent(
            """
            # Undo the upstream ElasticSearchEngine override.
            # See https://github.com/openedx/openedx-platform/issues/38991
            SEARCH_ENGINE = "search.typesense.TypesenseEngine"
            """
        )
    settings_override_config_map = kubernetes.core.v1.ConfigMap(
        f"ol-{stack_info.env_prefix}-edxapp-settings-override-{stack_info.env_suffix}",
        metadata={
            "name": settings_override_config_name,
            "namespace": namespace,
            "labels": k8s_global_labels,
        },
