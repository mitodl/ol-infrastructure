"""Deploy the application agent-class MCP workloads on the operations cluster.

This stack owns the ``toolhive-apps`` namespace: the ``MCPServer`` resources (and,
in a later iteration, ``VirtualMCPServer`` / ``MCPGroup`` resources) consumed by
application agents. The ToolHive operator and CRDs that reconcile these resources
...
TOOLHIVE_NAMESPACE = "toolhive-apps"
...
# MCPServer / VirtualMCPServer / MCPGroup resources for the application agent class
