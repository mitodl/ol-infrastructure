...
_min_replicas = xqwatcher_config.get_int("min_replicas")
min_replicas = 1 if _min_replicas is None else _min_replicas
...
xqwatcher_deployment = kubernetes.apps.v1.Deployment(
    ...
    opts=ResourceOptions(
        ...
        # Let the HPA manage replica count without Pulumi reverting it, but
        # only when an HPA actually exists (min_replicas > 0). When the
        # environment is intentionally disabled there's no HPA to defer to,
        # so Pulumi must keep enforcing replicas=0 against drift.
        ignore_changes=["spec.replicas"] if min_replicas > 0 else [],
    ),
)
...
#
# Kubernetes rejects minReplicas=0 for an HPA whose metrics are all
# Resource-type (CPU/memory) — scale-to-zero needs an external/object metric.
# So when an environment is deliberately disabled (min_replicas: 0, e.g. a
# stack with no live xqueue integration), skip the HPA entirely and leave the
# Deployment's static replicas=0 in place instead of fighting it back up.
if min_replicas > 0:
...
if edxorg_xqueue_enabled and edxorg_servers_secret and xqwatcher_edxorg_configmap:
    ...
    # See the equivalent guard above the main xqwatcher_hpa: Kubernetes
    # rejects minReplicas=0 for a Resource-metric-only HPA, so skip creating
    # this one too when the environment is disabled.
    if min_replicas > 0:
