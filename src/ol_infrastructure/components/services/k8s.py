...
class OLApplicationK8sConfig(BaseModel):
    ...
    webapp_vpa_max_allowed_memory: str | None = Field(
        ...
        description=(
            ...
            "CARVE-OUT, and check this before applying the rule: the above assumes the "
            "pod actually starts at the limit declared here in the manifest. A VPA whose "
            "admission controller is active mutates requests and limits at ADMISSION, so "
            "the pod never runs with the manifest value -- not even for an instant -- and "
            "the ceiling IS the current declared limit. Deriving the cap from "
            "resource_limits then sizes it against a number the kernel never enforces. "
            "mitxonline is the live example: its template says 1200Mi, its pods are "
            "admitted at 3Gi, kube_pod_container_resource_limits never once reported "
            "1200Mi over 14d, and p95 working set is ~2645MiB -- so the 'correct' "
            "1080MiB derivation would respawn the worker continuously at normal traffic. "
            "Confirm what the pods are actually admitted with before trusting either "
            "number. Tracking the limit as the VPA moves it requires reading the cgroup "
            "at runtime, tracked in "
