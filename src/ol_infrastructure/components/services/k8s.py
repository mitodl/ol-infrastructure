...
class OLApplicationK8sConfig(BaseModel):
    ...
    webapp_vpa_max_allowed_memory: str | None = Field(
        ...
        description=(
            ...
            "grown the pod near its ceiling. "
            "WHAT AN ACTIVE VPA ADMISSION CONTROLLER DOES AND DOES NOT CHANGE, because "
            "it is easy to talk yourself into a carve-out here that does not exist. It "
            "does mean the manifest value is not what most pods run with: requests and "
            "limits are mutated at admission, so resource_limits is a synth-time input, "
            "not an observed runtime limit. It does NOT mean pods are admitted AT the "
            "ceiling. maxAllowed bounds the recommendation; it does not pin it. A pod "
            "admitted while the recommendation is low starts low, so the admitted limit "
            "varies across pods and over time, and it is lowest for the newest pods -- "
            "precisely when a scale-up is adding them. "
            "Measured on mitxonline in production: over 14d the admitted limit ranged "
            "2838-3072MiB, which looks like it is pinned at the 3Gi ceiling; over 30d it "
            "ranged 1200-3072MiB, spending ~13 days' worth of samples below 2816MiB and "
            "sitting at exactly the 1200Mi VPA floor for ~30 hours. The 14d view is a "
            "non-representative slice. "
            "So the rule stands as written: size the cap below the LOWEST limit a pod can "
            "be admitted with, which is minAllowed (defaulted from resource_requests), "
            "not maxAllowed. A ceiling-derived cap is above the real cgroup limit for "
            "every freshly-admitted pod, which is exactly the disabled-safety-net case. "
            "Sizing it for the floor instead is usually not viable either -- mitxonline's "
            "p95 working set is ~2645MiB against a 1080MiB floor-derived cap -- so an app "
            "in this position has a genuine gap until the cap tracks the cgroup at "
            "runtime, tracked in "
            "tk-evaluate-runtime-cgroup-derived-workers-max-rss--1cd258. "
            "Raising minAllowed toward the ceiling is the other way to close it, at the "
            "cost of a guaranteed reservation per pod."
        ),
    )
