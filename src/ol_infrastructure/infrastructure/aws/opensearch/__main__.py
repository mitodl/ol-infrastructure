...
search_domain_access_policy = aws.elasticsearch.DomainPolicy(
    "opensearch-domain-access-policy",
    domain_name=search_domain.domain_name,
    access_policies=pulumi.Output.all(
        cidr=target_vpc["cidr"], arn=search_domain.arn
    ).apply(
        lambda kwargs: json.dumps(
            {
                ...
                "Statement": {
                    "Effect": "Allow",
                    "Action": ["es:ESHttp*"],
                    "Condition": {"IpAddress": {"aws:SourceIp": [kwargs["cidr"]]}},
                    "Resource": kwargs["arn"],
                },
            }
        )
