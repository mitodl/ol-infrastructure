...
consul_instance_data = {}
subnet_mappings = []

# Create EIPs
for count, subnet_id in zip(instance_range, subnet_ids):
    eip = ec2.Eip(f"consul-eip-{env_name}-{count}")  # different name?
    consul_instance_data[count] = {
        "subnet_id": subnet_id,
        "eip": eip,
    }
...
consul_nlb = lb.LoadBalancer(
    load_balancer_type="network",
    subnet_mappings=subnet_mappings,
    tags=aws_config.tags,
    # name spec? must be under 32 chars; env_name might be too long to include?
    # can be autogenerated
)

# TODO NLB Health Check + Listeners
...
consul_domain = route53.Record(
    ...
    type="CNAME",
    ttl=FIFTEEN_MINUTES,
    records=[consul_nlb.dns_name],
    zone_id=mitodl_zone_id,  # should this be consul_nlb.zone_id...?
)
...
# Auto Join WAN List
wan_list = []
for _, instance in consul_instance_data.items():
    eip = instance["eip"]
    wan_list.append(f"{eip}:{CONSUL_WAN_SERF_PORT}")
...
export("instances", consul_instance_data)
export("consul_lb", {"dns_name": consul_nlb.dns_name, "arn": consul_nlb.arn})
