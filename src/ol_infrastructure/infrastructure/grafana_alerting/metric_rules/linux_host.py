...
def create(
    ...
    # Concourse mounts every build volume and overlay from the root device, so
    # each one reports the same ratio as / and would otherwise become its own
    # alert instance. / still covers that disk.
    alerting.RuleGroup(
        ...
        rules=[
            alerting.RuleGroupRuleArgs(
                ...
                datas=rd(
                    '(host_filesystem_used_ratio{device=~"/dev.*",filesystem!~"(squashfs|vfat)",mountpoint!~"/var/concourse/worker/.+",job="integrations/linux_host"} * 100) > 80'
                ),
            ),
            alerting.RuleGroupRuleArgs(
                ...
                datas=rd(
                    '(host_filesystem_used_ratio{device=~"/dev.*",filesystem!~"(squashfs|vfat)",mountpoint!~"/var/concourse/worker/.+",job="integrations/linux_host"} * 100) > 95'
                ),
            ),
        ],
        opts=resource_opts,
