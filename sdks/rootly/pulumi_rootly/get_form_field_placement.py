# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from . import _utilities

__all__ = [
    'GetFormFieldPlacementResult',
    'AwaitableGetFormFieldPlacementResult',
    'get_form_field_placement',
    'get_form_field_placement_output',
]

@pulumi.output_type
class GetFormFieldPlacementResult:
    """
    A collection of values returned by getFormFieldPlacement.
    """
    def __init__(__self__, form=None, form_field_id=None, id=None):
        if form and not isinstance(form, str):
            raise TypeError("Expected argument 'form' to be a str")
        pulumi.set(__self__, "form", form)
        if form_field_id and not isinstance(form_field_id, str):
            raise TypeError("Expected argument 'form_field_id' to be a str")
        pulumi.set(__self__, "form_field_id", form_field_id)
        if id and not isinstance(id, str):
            raise TypeError("Expected argument 'id' to be a str")
        pulumi.set(__self__, "id", id)

    @_builtins.property
    @pulumi.getter
    def form(self) -> _builtins.str:
        return pulumi.get(self, "form")

    @_builtins.property
    @pulumi.getter(name="formFieldId")
    def form_field_id(self) -> _builtins.str:
        return pulumi.get(self, "form_field_id")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        return pulumi.get(self, "id")


class AwaitableGetFormFieldPlacementResult(GetFormFieldPlacementResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetFormFieldPlacementResult(
            form=self.form,
            form_field_id=self.form_field_id,
            id=self.id)


def get_form_field_placement(form: Optional[_builtins.str] = None,
                             form_field_id: Optional[_builtins.str] = None,
                             opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableGetFormFieldPlacementResult:
    """
    Use this data source to access information about an existing resource.
    """
    __args__ = dict()
    __args__['form'] = form
    __args__['formFieldId'] = form_field_id
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke('rootly:index/getFormFieldPlacement:getFormFieldPlacement', __args__, opts=opts, typ=GetFormFieldPlacementResult, package_ref=_utilities.get_package()).value

    return AwaitableGetFormFieldPlacementResult(
        form=pulumi.get(__ret__, 'form'),
        form_field_id=pulumi.get(__ret__, 'form_field_id'),
        id=pulumi.get(__ret__, 'id'))
def get_form_field_placement_output(form: Optional[pulumi.Input[Optional[_builtins.str]]] = None,
                                    form_field_id: Optional[pulumi.Input[Optional[_builtins.str]]] = None,
                                    opts: Optional[Union[pulumi.InvokeOptions, pulumi.InvokeOutputOptions]] = None) -> pulumi.Output[GetFormFieldPlacementResult]:
    """
    Use this data source to access information about an existing resource.
    """
    __args__ = dict()
    __args__['form'] = form
    __args__['formFieldId'] = form_field_id
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output('rootly:index/getFormFieldPlacement:getFormFieldPlacement', __args__, opts=opts, typ=GetFormFieldPlacementResult, package_ref=_utilities.get_package())
    return __ret__.apply(lambda __response__: GetFormFieldPlacementResult(
        form=pulumi.get(__response__, 'form'),
        form_field_id=pulumi.get(__response__, 'form_field_id'),
        id=pulumi.get(__response__, 'id')))
